...
function buildDashboardSheet(ss) {
  ...
  var overRule = SpreadsheetApp.newConditionalFormatRule()
    ...
    .setRanges([s.getRange("A10:F10"),s.getRange("A11:F11"),s.getRange("A12:F12")])
    .build();
  s.setConditionalFormatRules([overRule]);

  // -- DONUT CHART DATA (hidden at H1:J4) --
  s.getRange("H1").setValue("Category");
  s.getRange("I1").setValue("Amount");
  s.getRange("H2").setValue("Needs");   s.getRange("I2").setFormula("=D10");
  s.getRange("H3").setValue("Wants");   s.getRange("I3").setFormula("=D11");
  s.getRange("H4").setValue("Savings"); s.getRange("I4").setFormula("=D12");
  s.getRange("H1:I4").setFontColor("#6F746D"); // dim the helper data

  // Insert donut chart
  var chartData = s.getRange("H1:I4");
  var chart = s.newChart()
    .setChartType(Charts.ChartType.PIE)
    .addRange(chartData)
    .setPosition(8, 1, 0, 0) // row 8, col 1
